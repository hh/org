...
spec:
  ...
  resources:
    requests:
      storage: 1Gi
#+end_src
#+begin_src yaml :tangle ./sidero-controller-manager-tftp-patch.yaml
spec:
  template:
    spec:
      volumes:
...
#+begin_src shell
kubectl -n sidero-system get svc sidero-tftp
#+end_src

#+RESULTS:
#+begin_example
NAME          TYPE           CLUSTER-IP       EXTERNAL-IP    PORT(S)        AGE
sidero-tftp   LoadBalancer   10.106.255.147   192.168.1.21   69:31930/UDP   27h
#+end_example

Create a PVC for the TFTP folder
#+begin_src shell
kubectl apply -f ./sidero-controller-manager-tftp-pvc.yaml
kubectl -n sidero-system patch deployment sidero-controller-manager --patch-file ./sidero-controller-manager-tftp-patch.yaml
#+end_src
...
dhcp-boot=ipxe-arm64.efi,192.168.1.127,192.168.1.127
