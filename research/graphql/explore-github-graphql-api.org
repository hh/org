...
The current version of the GitHub API (v4) uses a single end point. This is a quaility of the GraphQL API, and what makes it disinct from the REST API. You will have a single endpoint, but it can take any kind of query that matches the API schema.
