...
## Load csv to bq
bq load --autodetect "${GCP_BIGQUERY_DATASET}.potaroo_all_asn_name" /tmp/potaroo_asn.txt
...
for VENDOR in ${VENDORS[*]}; do
  ...
  bq load --autodetect "${GCP_BIGQUERY_DATASET}.vendor_yaml" "/tmp/vendor/${VENDOR}_yaml.csv"
done
...
for VENDOR in ${ASN_VENDORS[*]}; do
  bq load --autodetect "${GCP_BIGQUERY_DATASET}.vandor_json" "/tmp/vendor/${VENDOR}_raw_subnet_region.csv"
done
