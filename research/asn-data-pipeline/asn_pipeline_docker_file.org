...
docker run \
    ...
    -e GCP_BIGQUERY_DATASET=etl_script_generated_set_1 \
    -e GCP_BIGQUERY_DATASET_BK=etl_script_generated_set \
...
bq load --autodetect ${GCP_BIGQUERY_DATASET}.06_ip_range_2_ip_lookup /tmp/match-ip-to-iprange.csv
#+end_src

Stop the database
#+begin_src shell :tangle ./pg-init.d/12-stop-postgres.sh
