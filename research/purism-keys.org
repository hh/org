...
# eval: (set (make-local-variable 'start-tmate-command) (concat "tmate -S " socket " new-session -A -s " user-login-name " -n main \"tmate wait tmate-ready && tmate display -p '#{tmate_ssh} # " user-buffer ".target # #{tmate_web} ' | xclip -i -sel p -f | xclip -i -sel c; sleep 0.05 ;  xclip -o ; echo Have you pasted? ; read ;  bash --login\""))
# eval: (xclip-mode 1)
